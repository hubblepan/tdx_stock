...
def get_annotation_data():
    import re
    with open('{}/{}'.format('data', '2_sma_track.txt'), 'r', encoding='utf-8') as f:
...
def get_phone_data(key):
    ...
    if '{}.blk'.format(key) in file_list:
        ...
        with open('{}/{}'.format('data', '2_sma_track.txt'), 'w', encoding='utf-8') as f:
            for line in lines:
...
if __name__=='__main__':
    #data to phone
    stock_code_list = []
    with open('{}/{}'.format('data', 'today.txt'), 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            if not line.startswith("#") and not '\n' == line:
                data = line.strip('\n').split(',')
                if data[0] not in stock_code_list:
                    stock_code_list.append(data[0][1:])
